...
// FIX: Explicitly typed Toast component with React.FC to prevent errors with the 'key' prop.
...
const calculateMinutes = (startTime, endTime) => {
  ...
  // FIX: Use .getTime() to pass a number to isNaN, satisfying TypeScript's type checker.
...
const exportToPDF = (exportData, filename = 'report.pdf') => {
  const { entries, annualHolidayAllowance, holidays, leaveTypes, leaveEntries } = exportData;
  // FIX: Cast window to any to access jspdf property which is loaded via a script tag.
  ...
  doc.autoTable({
    ...
    didDrawCell: (data) => {
        if (data.section === 'body' && data.column.index === 1) {
            ...
            let color;
            if (entry.type === 'holiday') {
                color = HOLIDAY_COLOR.hex;
            } else {
                color = getColorForLeaveType(leaveTypes, entry.leaveType.id).hex;
            }
            doc.setFillColor(color);
            doc.rect(data.cell.x + 2, data.cell.y + data.cell.height / 2 - 1.5, 3, 3, 'F');
            data.cell.textPos.x = data.cell.x + 8; // Offset text to avoid overlap
        }
    },
  });
...
// FIX: Explicitly typed SummaryCard component with React.FC to prevent errors with the 'key' prop.
const SummaryCard: React.FC<{iconName: string, title: string, value: string, colorClass: string, onAdd?: () => void}> = ({ iconName, title, value, colorClass, onAdd }) => (
  <div className="relative bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-md flex items-center space-x-4">
    {onAdd && (
      <button 
        onClick={onAdd}
        className="absolute top-2 right-2 p-1.5 bg-slate-100 dark:bg-slate-700 rounded-full text-slate-500 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600 hover:text-blue-500 dark:hover:text-blue-400 transition-colors"
          ...
          {filteredEntries.length > 0 ? (
            filteredEntries.map((entry, index) => {
              const entryColor = entry.type === 'holiday' 
                ? HOLIDAY_COLOR
                : getColorForLeaveType(leaveTypes, entry.leaveType.id);
              
              return (
                <div key={index} className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-md flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
...
const AddEntryModal = ({ isOpen, onClose, onSave, leaveTypes, existingEntry, initialData, remainingHolidays, leaveBalances }) => {
...
const SettingsModal = ({ isOpen, onClose, settings, onSaveSettings }) => {
  ...
  const handleImport = (event) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        try {
          // FIX: Cast FileReader result to string before parsing, as readAsText is used.
...
const App = () => {
  ...
  const remainingHolidays = settings.annualHolidayAllowance - holidays.length;
  const leaveBalances = useMemo(() => {
  ...
  }, [settings.leaveTypes, leaveEntries]);
  ...
  return (
    <ToastContext.Provider value={{ success: (msg) => addToast(msg, 'success'), error: (msg) => addToast(msg, 'error'), info: (msg) => addToast(msg, 'info') }}>
      ...
      <SettingsModal
        ...
        settings={{ ...settings, holidays, leaveEntries }}
      />
